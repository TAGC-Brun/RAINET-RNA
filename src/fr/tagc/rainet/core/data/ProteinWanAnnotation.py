...
class ProteinWanAnnotation( Base ):
    ...
    def __init__(self, wan_cluster_id, protein_id):
        ...
        #=======================================================================
        # Get the ProteinCrossReference corresponding to the provided protein ID
        ...
        if protein_crossref_list == None or len( protein_crossref_list) == 0:
            raise NotRequiredInstantiationException( "ProteinWanAnnotation.init : No ProteinCrossReference found for cross reference = " + protein_id)
        ...
        for cross_ref in protein_crossref_list:
            ...
            if protein_list != None and len( protein_list) > 0:
                if len( protein_list) == 1:
                    ...
                    raise RainetException( "ProteinWanAnnotation.init : Abnormal number of Protein found for cross reference = " + cross_ref.protein_id + " : " + str( len( protein_list))) 
            else:
                raise NotRequiredInstantiationException( "ProteinWanAnnotation.init : No Protein found for uniprotAC = " + cross_ref.protein_id)
     
            # -- Check if the Protein found is not None
            if protein == None:
                raise RainetException( "ProteinWanAnnotation.init : returned Protein is None for UniprotAC" + cross_ref.protein_id)
